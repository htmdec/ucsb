...
from .indentation_analysis_main import Average, get_sub, get_super, CMAP_Hardness, CMAP_SRS, CMAP_Vact

def indentation_analysis_run(Row, Col, color, b, a, J1, SR_diff, T, Kb, Burgers_Vector, Z_path, dest_path):
        ...
        for y in range (0,Col): # Column selection
            ...
            while True: # Test selection
                ...
                try: # Routine that checks if the next file exist. If not, generates an error message and prints the number of tests
                    with open(Z_path + str(x) + '.' + str(y) + '_T' + str(i)+ '.CSV'):pass
                except IOError:
